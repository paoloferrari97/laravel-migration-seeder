        ...
        <form action="{{ route('packages.update', $package->id) }}" method="post">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control @error('destinazione') is-invalid @enderror" id="destinazione"
                    name="destinazione" placeholder="Inserisci qui la destinazione!" value="{{ $package->destinazione }}">
            </div>
            @error('destinazione')
            ...
            <div class="form-group">
                ...
                <input type="number" step="0.01" class="form-control @error('prezzo') is-invalid @enderror" id="prezzo"
                    name="prezzo" placeholder="Inserisci qui il prezzo!" value="{{ $package->prezzo }}">
            </div>
            @error('prezzo')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
            ...
            <div class="form-group">
                ...
                <input type="date" class="form-control @error('data_partenza') is-invalid @enderror" id="data_partenza"
                    name="data_partenza" value="{{ $package->data_partenza }}">
            </div>
            @error('data_partenza')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
